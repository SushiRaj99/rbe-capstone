...
class Nav2GymEnv(gym.Env):
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None) -> Tuple[np.ndarray, dict]:
        super().reset(seed=None)    # for some reason super().reset(seed=seed) was crashing when seed is not None
        ...
        if (reset_applied): raw_observation = self.bridge.wait_for_observation(timeout=30.0)    # allow good chunk of time for map load
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, dict]:
        ...
        raw_observation = self.bridge.wait_for_observation(timeout=15.0)
        if raw_observation is None:
            ...
            #self.bridge.get_logger().info("Observation received!")
            observation = self.normalize_observation(raw_observation)
            status = self.bridge.get_status()
            reward, info = self.compute_step_reward(raw_observation, status)
            terminated = status in ['goal_reached', 'collision', 'stuck']
    ...
    def normalize_observation(self, raw_obs: np.ndarray) -> np.ndarray:
        ...
        obs[(2*putils.N_LIDAR_RAYS) + 0] = np.clip(obs[(2*putils.N_LIDAR_RAYS) + 0] / putils.MAX_GOAL_DIST, -1.0, 1.0)
        # Y distance to goal [-MAX_GOAL_DIST, MAX_GOAL_DIST] -> [-1, 1]:
        obs[(2*putils.N_LIDAR_RAYS) + 1] = np.clip(obs[(2*putils.N_LIDAR_RAYS) + 1] / putils.MAX_GOAL_DIST, -1.0, 1.0)
        # Bearing to goal [-pi, pi] -> [-1, 1]
        obs[(2*putils.N_LIDAR_RAYS) + 2] = np.clip(obs[(2*putils.N_LIDAR_RAYS) + 2] / np.pi, -1.0, 1.0)
        # Forward velocity [-MAX_VX, MAX_VX] -> [-1, 1]:
        obs[(2*putils.N_LIDAR_RAYS) + 3] = np.clip(obs[(2*putils.N_LIDAR_RAYS) + 3] / putils.MAX_VX, -1.0, 1.0)
        # Steering velocity [-MAX_VX, MAX_VX] -> [-1, 1]:
        obs[(2*putils.N_LIDAR_RAYS) + 4] = np.clip(obs[(2*putils.N_LIDAR_RAYS) + 4] / putils.MAX_WZ, -1.0, 1.0)
        return obs.astype(np.float32)

    def compute_step_reward(self, raw_obs: np.ndarray, status: str) -> Tuple[float, dict]:
...
def evaluate_baseline(
    ...
    for episode in range(num_episodes):
        ...
        # Inner episode loop until termination or truncation:
        while not done:
            raw_observation = bridge.wait_for_observation(timeout=15.0)  # wait for next observation WITHOUT publishing an action
            if raw_observation is None:
                ...
                terminated = episode_status in ['goal_reached', 'collision', 'stuck']
                truncated = 'timeout' in episode_status
                step_reward, step_info = env.compute_step_reward(raw_observation, episode_status)
...
def summarize_eval_results(label: str, rewards: List[float], lengths: List[float], outcomes: List[str]) -> dict:
    ...
    stuck_rate = sum(1 for o in outcomes if 'stuck' in o) / n * 100.0
    summary = dict(
        label          = label,
        n_episodes     = n,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument('--ro-buffer', type=int, default=512, help='Rollout buffer size for PPO training')
    p.add_argument('--batch-size', type=int, default=32, help='Batch size for PPO training')
