...
class PlannerConfigManager(Node):
    def __init__(self):
        ...
        self.reset_pub = self.create_publisher(EmptyMsg, '/rl/reset_event', 10)
        ...
        self.curr_goal_stuck: bool = False
    ...
    def reset_config(self, request: Empty.Request, response: Empty.Response) -> Empty.Response:
        ...
        self.reset_pub.publish(EmptyMsg()) # publish to rl/reset_event to notify the lidar_model to update its map
        return response
    ...
    def send_nav2_goal(self, x: float, y: float, yaw: float, xy_tol: float, yaw_tol: float) -> None:
        if not self.goal_action_client.wait_for_server(timeout_sec=20.0):
    ...
    def process_goalmanager_result(self, future: Future) -> None:
        ...
        if (result.goal_reached) or (result.nav2_status == "SUCCEEDED"):
            self.curr_goal_reached = True
        else:
            self.curr_goal_stuck = True
    ...
    def publish_observation(self) -> None:
        ...
        if self.episode_status == 'idle':
            #self.get_logger().warn("Can't build observation since the episode is 'idle'")
            return
        ...
        #self.get_logger().info("Attempting to build observation...")
    ...
    def check_terminal(self) -> str:
        ...
        if (self.latest_ranges is not None): 
            min_range = float(np.min(self.latest_ranges[0::2]))  # ranges only, skip bearings
            if (min_range < collision_thresh):
                return 'collision'
        ...
        if (self.curr_goal_reached):
            return 'goal_reached'
        # Check for "timeout" (max number of steps exceeded) or "stuck" (when Nav2 says it's done for any other reason than "SUCCEEDED"):
        if (self.curr_goal_stuck):
            return 'stuck'
        elif self.episode_step >= max_steps:
            return 'timeout'
    ...
    def build_observation(self) -> Optional[np.ndarray]:
        ...
        #   ]
        observation = None
        #self.get_logger().info("Checking for valid scans...")
        if (self.latest_ranges is not None) and (len(self.latest_ranges) > 0):
            #self.get_logger().info("Valid scans found!")
            pose = self.get_robot_pose()
            if pose is not None:
                x, y, yaw = pose
