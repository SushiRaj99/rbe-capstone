...
class ParamDialNode(Node):
    def __init__(self):
        ...
        self.create_subscription(String, '/potr_navigation/current_planner_params', self.on_params, latched_qos)
...
def draw(ax, snapshot):
    ax.clear()
    if snapshot is None:
        ax.text(0.5, 0.5, 'waiting for planner_controller...', ha='center', va='center', transform=ax.transAxes, fontsize=12, color='gray')
        ax.set_xticks([])
        ax.set_yticks([])
    ...
    for i, name in enumerate(names):
        ...
        ax.plot([0, 1], [y, y], color='lightgray', linewidth=12, solid_capstyle='round', zorder=1)
        ax.plot([nbase], [y], marker='|', color='dimgray', markersize=22, markeredgewidth=2, zorder=3)

        if nval is not None:
            colour = 'steelblue' if nval >= nbase else 'darkorange'
            ax.plot([0, nval], [y, y], color=colour, linewidth=12, solid_capstyle='round', zorder=2)
            ax.plot([nval], [y], marker='o', color=colour, markersize=12, markeredgecolor='white', markeredgewidth=1.5, zorder=4)

        ax.text(-0.02, y, name, ha='right', va='center', fontsize=10)
        label = f'{val:.3f}' if val is not None else '-'
        ax.text(1.02, y, label, ha='left', va='center', fontsize=10, family='monospace')
        ax.text(0.0, y - 0.38, f'{lo:g}', ha='center', fontsize=7, color='gray')
        ax.text(1.0, y - 0.38, f'{hi:g}', ha='center', fontsize=7, color='gray')
