...
\\section{Materials and Methods}\label{sec:pq_metrics}
    ...
    The reward function was then expanded to include the metrics described above for path quality, such as applying a small negative reward for high
    path curvature or large changes in velocity to encourage smoother paths. Since the state vector is a lower dimensionality and more closely tied
    to interactions with the environment, the RL agent will have more feedback during each episode to learn from and may converge to an optimal policy
    more quickly than the one-time tuning approach. The dynamic tuning ``hands on'' nature should make this deployment more malleable from an RL
    training perspective, as it provides the agent the opportunity to learn from each time step during navigation within a single episode. Regardless
    of the method used to translate a static map into a state vector, the state vector of the dynamic tuning approach is much more similar to the
    inputs of the local planner than the state vector of the one-time tuning approach. This facilitates easier learning and decision making for the
    RL agent since there's already a strongly established correlation between parameters in the state vector and the performance of the local planner
    that the RL agent is trying to optimize. Additionally, the actions for the dynamic tuning approach may be more forgiving than the action for the
    one-time tuning approach, which leads to more stable results and the ability to walk back any missteps.
