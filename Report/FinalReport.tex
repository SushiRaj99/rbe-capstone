...
\section{Materials and Methods}\label{sec:pq_metrics}
    ...
    throughout the navigation process where the state would includes the robot's current pose, velocity, distance from the goal, the obstacle cost of the surrounding environment. The actions continously adjust the parameters (continuous mode) at each timestep. The reward function is be designed to encourage efficient, smooth, and safe navigation by providing positive rewards for progress towards the goal and negative rewards for collisions, long travel times, and time spent in close proximity to obstacles. The goal of these rewards is to encourage the agent to learn how to guide the robot through crowded environments as smoothly and quickly as possible.
    ...
    % Depending on how the "Performance Evaluation" builds out (see respective note in that sub-sub section), the gymnasium environment could either be introduced in it's own subsubsection here, or it could be introduced within the "Training Architecture" section below. Either way, it should be talked about.
        ...
        Checkpoints are saved every 2000 environment steps using SB3's \texttt{CheckpointCallback}. The replay buffer is saved
        ...
        The reward function is built around five per-step terms and two terminal terms. Each term targets one of the four navigation
        quality criteria: efficiency, smoothness, accuracy, and clearance.
        ...
        \end{itemize}
        % Not sure exactly what Sushant's latest version does (don't think it's uploaded to his branch yet), but if these are all baked into the reward function, the actual "performance evaluation" would be assessing and comparing the total rewards of the planners as a sort of score. Benefit to this is that it's a concrete single score and might minimize the amount of individual analysis needed for each criteria. If Sushant's latest branch doesn't already include it, it shouldn't be too hard to write a dedicated script to run a baseline planner within the gymnasium environment (you either fake an action at every step, or just compute a reward for each observation that builds up in the queue).
