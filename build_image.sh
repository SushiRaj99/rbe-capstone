...
while [ $# -ge 1 ]; do
    case $1 in
        ...
        '-l'|'-L'|'--load')
            ...
            ;;
        '-h'|'--help')
            echo "build_image.sh contains the following optional inputs:"
            ...
            ;;
        *)
            echo "ERROR: Invalid input argument. Seek help via 'build_image.sh --help'"
            exit 1
            ;;
    esac
    shift;
...
docker buildx build . -f docker/rbe_capstone.dockerfile --network=host --platform linux/amd64,linux/arm64 -t ghcr.io/ecwenzlaff/rbe-capstone:${VERSION} ${loadcmd[@]}
